...
class Ui_Dialog(QtGui.QWidget):
    ...
    def retranslateUi(self, Dialog):
        """
        Bind ui elements with their functions
        """
    ...
    def docBrowse(self):
        """
        Browse and set path of target document
        """
    ...
    def corpBrowse(self):
        """
        Browse and set path to corpus folder
        """
        self.corpus_path = QtGui.QFileDialog.getExistingDirectory(None, 'Select a folder:', 'C:/', QtGui.QFileDialog.ShowDirsOnly)
        self.pushButton_2.setText("Corpus: " + self.corpus_path)
        print(self.corpus_path)

    def runCode(self):
        """
        Run plagiarism computation code
        """
        if self.target_path is None or len(self.target_path) < 1:
            print('Please select a target document for checking!')
            return

        if self.static.isChecked():
            if self.corpus_path is None or len(self.corpus_path) < 1:
    ...
    def dynamicClicked(self):
        """
        Hide unnecessary fields if dynamic search selected.
        """
    ...
    def staticClicked(self):
        """
        Show all fields if static search selected.
        """
